...
function Get-BranchesFromAppCenter {
    param (
    ...
    {
        $global:branches = $requestResult.Content | ConvertFrom-Json
    } else {
...
function Get-BuildStatus {
    Get-BranchesFromAppCenter -ownerName $ownerName -appName $appName -token $token
...
switch ($reqOperation)
{
    "GetStatus" {Get-BuildStatus}
    "NewBuild" {Get-NewBuild}
    default {
